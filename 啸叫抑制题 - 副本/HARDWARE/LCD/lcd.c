...
{ 	
  ...
  FSMC_NORSRAMCmd(FSMC_Bank1_NORSRAM4, ENABLE);  // 使能BANK1 
	...
	LCD_Display_Dir(LCD_DIRECTION);		//默认为竖屏   //0竖屏 1横屏
	LCD_LED=1;				//点亮背光
	LCD_Clear(WHITE);
}  
//清屏函数
...
{
	LCD_ShowxNum(x,y,(u32)num,len1,size,0);
	if(len2)
	{
	LCD_ShowChar(x+len1*size/2,y,'.',size,0);
	LCD_ShowxNum(x+(len1+1)*size/2,y,(num-(u32)num)*LCD_Pow(10,len2),len2,size,1<<7);
	}
}
//显示数字,高位为0,还是显示
//x,y:起点坐标
...
void LCD_ShowString_Color(u16 x,u16 y,u16 width,u16 height,u8 size,u16 color,u8 *p)
{         
	u8 x0=x;
	width+=x;
	height+=y;
